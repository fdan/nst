...
# todo: this has no noticeable effect
class HistogramGuide(OptGuide):
    ...
    def prepare(self):
        ...
        b = self.tensor[0][1].sum() / x * y
        g = self.tensor[0][2].sum() / x * y
        self.target = torch.Tensor([r.item(), g.item(), b.item()])
    ...
    def forward(self, optimiser, opt_tensor, loss, iteration):
        ...
        _, c, x, y = opt_tensor.size()
        r = opt_tensor[0][0].sum() / x * y
        g = opt_tensor[0][1].sum() / x * y
        b = opt_tensor[0][2].sum() / x * y
        opt_hist = torch.Tensor([r.item(), g.item(), b.item()])
