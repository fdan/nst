...
const char* const MLClient::kDefaultHostName = "172.22.15.27";
  ...
  mlserver::RespondWrapper& responseWrapper, std::string& errorMsg)
{
  ...
  try {
    ...
    // dan: it's an issue that we're not doing this per image.  i.e. here
    // we embed the assumption all images have the same format...
    ...
    for (int i = 0; i < node_inputs(); i++) {
      ...
      // dan: this sounds ideal, we are getting the info per image
      Box imageBounds = inputIop->info();

//      std::stringstream ss3;
//      ss3 << "\nr: " << std::to_string(imageBounds.r());
//      ss3 << "\nt: " << std::to_string(imageBounds.t());
//      ss3 << "\n";
...
//      size_t byteBufferSize = fr * ft * kDefaultNumberOfChannels * sizeof(float);
      if (byteBufferSize == 0) {
